                        ...
                        <div className="space-y-4">
                            ...
                            <div className="glass-card p-5 space-y-4">
                                ...
                                <div className="space-y-3 mt-6 pt-6 border-t border-surface-border">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <label className="text-sm font-medium text-slate-800 block">Lokal Printerlar</label>
                                            <p className="text-xs text-slate-400">Oshxona yoki bar printerlarini mahalliy IP orqali qo'shing.</p>
                                        </div>
                                        <button
                                            onClick={() => {
                                                const currentPrinters = ubtDraft?.printers || [];
                                                setUbtDraft({ ...ubtDraft, printers: [...currentPrinters, { id: Date.now().toString(), name: "Yangi printer", ip: "192.168.1.100", type: "kitchen" }] })
                                            }}
                                            className="text-xs text-amber-500 hover:text-amber-400 font-semibold flex items-center gap-1 bg-amber-500/10 px-2 py-1 rounded-md transition"
                                        >
                                            <Printer size={12} /> Printer qo'shish
                                        </button>
                                    </div>
                                    <div className="space-y-3">
                                        {(ubtDraft?.printers || []).map((printer: any, i: number) => (
                                            <div key={printer.id} className="flex flex-col sm:flex-row items-center gap-3 bg-surface-elevated p-3 rounded-xl border border-surface-border">
                                                <div className="w-full sm:flex-1 space-y-1">
                                                    <label className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Printer nomi</label>
                                                    <input value={printer.name} onChange={e => {
                                                        const newPrinters = [...(ubtDraft?.printers || [])];
                                                        newPrinters[i].name = e.target.value;
                                                        setUbtDraft({ ...ubtDraft, printers: newPrinters });
                                                    }} className="input-field w-full h-9 text-sm" placeholder="Oshxona printeri" />
                                                </div>
                                                <div className="w-full sm:flex-1 space-y-1">
                                                    <label className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">IP Manzil</label>
                                                    <input value={printer.ip} onChange={e => {
                                                        const newPrinters = [...(ubtDraft?.printers || [])];
                                                        newPrinters[i].ip = e.target.value;
                                                        setUbtDraft({ ...ubtDraft, printers: newPrinters });
                                                    }} className="input-field w-full h-9 text-sm font-mono text-brand" placeholder="192.168.1.x" />
                                                </div>
                                                <div className="w-full sm:w-40 space-y-1">
                                                    <label className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Printer turi</label>
                                                    <select value={printer.type} onChange={e => {
                                                        const newPrinters = [...(ubtDraft?.printers || [])];
                                                        newPrinters[i].type = e.target.value;
                                                        setUbtDraft({ ...ubtDraft, printers: newPrinters });
                                                    }} className="input-field w-full h-9 text-sm">
                                                        <option value="kitchen">Oshxona</option>
                                                        <option value="bar">Bar</option>
                                                        <option value="receipt">Kassa</option>
                                                        <option value="other">Boshqa</option>
                                                    </select>
                                                </div>
                                                <button onClick={() => {
                                                    const newPrinters = (ubtDraft?.printers || []).filter((_: any, idx: number) => idx !== i);
                                                    setUbtDraft({ ...ubtDraft, printers: newPrinters });
                                                }} className="text-red-400 hover:text-red-300 p-2 sm:mt-5 rounded-lg hover:bg-red-400/10 transition-colors w-full sm:w-auto flex justify-center">
                                                    <X size={16} />
                                                </button>
                                            </div>
                                        ))}
                                        {(ubtDraft?.printers || []).length === 0 && (
                                            <div className="text-center py-6 border border-dashed border-surface-border rounded-xl text-slate-500 text-sm">
