    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-6 space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-3 gap-5">
                ...
                <div className="col-span-1 bg-slate-800/50 backdrop-blur border border-slate-700/50 rounded-2xl p-5">
                ...
                {/* Entries table */}
                <div className="col-span-2 bg-slate-800/50 backdrop-blur border border-slate-700/50 rounded-2xl p-5">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="text-slate-800 font-bold">Kassa Harakatlari</h3>
                        <div className="flex gap-2">
                            {(["all", "income", "expense"] as const).map(tab => (
                                <button key={tab} onClick={() => setActiveTab(tab)}
                                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${activeTab === tab ? "bg-blue-500 text-white" : "text-slate-400 hover:text-slate-200 bg-slate-700/50"}`}>
                                    {tab === "all" ? "Barchasi" : tab === "income" ? "✅ Kirimlar" : "❌ Xarajatlar"}
                                </button>
                            ))}
                            <button onClick={fetchData} className="p-1.5 text-slate-400 hover:text-white bg-slate-700/50 rounded-lg transition">
                    ...
                    <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-md p-6 animate-in zoom-in-95 duration-200">
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-3">
                                ...
                                <h3 className="text-white font-bold text-lg">{modalType === "income" ? "Kirim Qo'shish" : "Xarajat Qo'shish"}</h3>
                            </div>
                        ...
                        <form onSubmit={handleSave} className="space-y-4">
                            <div>
                                ...
                                <select required value={form.category} onChange={e => setForm({ ...form, category: e.target.value })}
                                    className="w-full mt-1.5 px-4 py-2.5 bg-slate-800 border border-slate-600 rounded-xl text-white text-sm outline-none focus:border-blue-500 transition">
                            ...
                            <div>
                                        ...
                                        <button key={key} type="button"
                                            ...
                                            <div className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all
                                                ${ kontragentType === key
                                                    ...
                                                    : `bg-gradient-to-br ${bg} opacity-70 group-hover:opacity-100` }`}>
                                                <Icon size={16} className="text-white" />
                                            </div>
                                            <span className={`text-[10px] font-bold tracking-wide transition-colors
                                                ${ kontragentType === key ? "text-white" : "text-slate-400 group-hover:text-slate-200" }`}>
                                                {label}
                                            </span>
                                        </button>
                                    ))}
                                ...
                                {kontragentType === "boshqa" && (
                                    <input type="text" placeholder="Kontragent nomini kiriting..."
                                        ...
                                        className="w-full mt-2 px-4 py-2.5 bg-slate-800 border border-slate-600 rounded-xl text-white text-sm outline-none focus:border-blue-500 transition" />
                                )}
                            ...
                            <div className="flex gap-3 pt-2">
                                <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 px-4 py-2.5 bg-slate-800 text-slate-300 rounded-xl font-bold text-sm hover:bg-slate-700 transition border border-slate-600">
                                    Bekor
                                </button>
