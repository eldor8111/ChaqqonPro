...
import { Plus, Search, Edit2, Trash2, X, Users, Phone, Star, UserPlus } from "lucide-react";
...
const emptyForm = { name: "", phone: "", segment: "Normal", bonusPoints: 0 };
...
async function createCustomer(body: typeof emptyForm) {
    const res = await fetch("/api/customers", {
        method: "POST",
...
async function updateCustomer(id: string, body: typeof emptyForm) {
...
export default function KlientlarPage() {
    const { t } = useLang();
    ...
    const updateMutation = useMutation({
        mutationFn: ({ id, data }: { id: string; data: typeof emptyForm }) => updateCustomer(id, data),
        onSuccess: () => { qc.invalidateQueries({ queryKey: ["customers"] }); closeForm(); },
        onError: (e: any) => setErrorMsg(e.message),
    });
    ...
    const deleteMutation = useMutation({
        ...
        onSuccess: () => { qc.invalidateQueries({ queryKey: ["customers"] }); setDeleteConfirmId(null); },
    });
    ...
    const openEdit = (c: Customer) => {
        setEditingId(c.id);
        setForm({ name: c.name, phone: c.phone || "", segment: c.segment || "Normal", bonusPoints: c.bonusPoints || 0 });
        setErrorMsg("");
        setShowForm(true);
    };
        ...
        <>
            <div className="space-y-5 animate-fade-in">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-bold text-slate-800">{t('nav.kont_clients') || 'Mijozlar'}</h1>
                        <p className="text-sm text-slate-400 mt-1">{customers.length} ta yetkazilayotgan mijoz</p>
                    </div>
                    <button
                        onClick={openNew}
                        className="btn-primary flex items-center gap-2"
                    >
                        <Plus size={16} /> {t('common.add')} Mijoz
                    </button>
                </div>
                    ...
                    <div className="glass-card p-4 rounded-xl border border-surface-border">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-400">Jami mijozlar</p>
                            </div>
                        </div>
                    </div>
                    <div className="glass-card p-4 rounded-xl border border-surface-border">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-400">VIP Mijozlar</p>
                            </div>
                        </div>
                    </div>
                    <div className="glass-card p-4 rounded-xl border border-surface-border">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-400">Telefonli kontaktlar</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="relative max-w-sm">
                    ...
                    <input
                        ...
                        placeholder="Mijoz ismi, telefon orqali izlash..."
                ...
                <div className="glass-card overflow-x-auto">
                    <table className="data-table min-w-[700px]">
                        ...
                        <tbody>
                            {isLoading ? (
                                <tr><td colSpan={6} className="text-center text-slate-500 py-10">Yuklanmoqda...</td></tr>
                            ) : filtered.length === 0 ? (
                                <tr>
                                    <td colSpan={6} className="text-center py-14">
                                        <div className="flex flex-col items-center gap-3 text-slate-400">
                                            ...
                                            <p className="text-sm font-medium">
                                                {search ? "Qidiruv bo'yicha mijoz topilmadi" : "Hali mijoz qo'shilmagan"}
                                            </p>
                                            {!search && (
                                ...
                                <tr key={c.id}>
                                    ...
                                    <td className="text-slate-500 text-sm">{c.phone || <span className="text-slate-300">—</span>}</td>
                                    <td>
                                        <span className={`px-2 py-0.5 rounded-md text-xs font-medium ${
                                            c.segment === "VIP" ? "bg-amber-100 text-amber-700" :
                                            c.segment === "New" ? "bg-emerald-100 text-emerald-700" :
                                    ...
                                    <td className="text-slate-500 text-sm font-medium">
                                        {c.bonusPoints?.toLocaleString() || 0} ball
                                    </td>
                                    <td>
                    ...
                    <div className="relative z-10 w-full max-w-md bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-slate-200">
                        ...
                        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 flex-shrink-0">
                            <h2 className="font-semibold text-gray-800 text-[15px]">
                                {editingId ? "Mijozni tahrirlash" : "Yangi mijoz qo'shish"}
                            </h2>
                        ...
                        <div className="overflow-y-auto px-5 py-5 space-y-4">
                            ...
                            <div>
                                <label className="block text-xs text-gray-500 mb-1.5 font-medium">Segment</label>
                                <select
                                    value={form.segment}
                ...
                <div className="fixed inset-0 z-[999] flex items-start justify-center pt-[20vh] px-4">
                    ...
                    <div className="relative z-10 bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4 animate-in fade-in zoom-in-95 duration-200 border border-slate-200">
                        ...
                        <p className="text-slate-500 text-sm mb-6">
                            Ushbu mijoz jadvaldan butunlay o&apos;chirib tashlanadi. Bu amalni qaytarib bo&apos;lmaydi.
                        </p>
