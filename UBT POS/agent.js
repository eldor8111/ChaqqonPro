...
async function printRaw(printerName, base64data, options = {}) {
    ...
    if (printerIp) {
        try {
            ...
            console.log(`[PECHAT OK LAN] ${printerIp}:${printerPort} ga chop etildi.`);
        } catch (err) {
...
`;
    ...
    try {
        ...
        console.log(`[PECHAT OK USB] "${printerName}" ga chop etildi.`);
    } catch(err) {
...
console.log("  USB + LAN (TCP/IP 9100) ishlaydi      ");
