...
// Removed mockData imports to initialize with empty state
...
export interface AppState {
    kpi: { todaySales: number; totalRevenue: number; activeOrders: number; conversion: string };
    revenueChart: any[];
    topProducts: any[];
    ...
    updateProduct: (id: string, product: Partial<any>) => void;
...
export const useStore = create<AppState>()(
    persist(
        (set) => ({
            kpi: { todaySales: 0, totalRevenue: 0, activeOrders: 0, conversion: "0%" },
            revenueChart: [],
            topProducts: [],
            recentTransactions: [],
            products: [],
            pharmacyDrugs: [] as PharmacyDrug[],
            onlineOrders: [],
            profitLoss: [],
