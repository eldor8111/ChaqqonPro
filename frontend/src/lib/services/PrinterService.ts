...
const cmd = {
    ...
    // ESC G 1 — double-strike: har bir nuqtani 2 marta bosadi → qoʻyuqroq bosma
    dark:      () => Buffer.from([ESC, 0x47, 0x01]),
...
async function getReceiptOpts(tenantId?: string): Promise<ReceiptOpts> {
    ...
    try {
        ...
        const opts: ReceiptOpts = {
            ...
            shopNameFontSize: Number(r.shopNameFontSize) || 16,
        ...
        return { 
            shopName: "RESTORAN", customShopName: "", shopNameFontSize: 16, shopNameAlign: "center", shopNameBold: true,
...
function buildKitchenBuffer(job: PrintJob, opts: ReceiptOpts): Buffer {
    ...
    parts.push(dashed(), line(""), line(""), line(""), cmd.cut());
...
function buildClientBuffer(job: PrintJob, opts: ReceiptOpts): Buffer {
    ...
    parts.push(line(""), line(""), line(""), cmd.cut());
    return Buffer.concat(parts);
}
...
function buildReportBuffer(job: PrintJob, opts: ReceiptOpts): Buffer {
    ...
    parts.push(dashed(), line(""), line("Xizmat ko'rsatganingiz uchun rahmat!"), line(""), line(""), line(""), cmd.cut());
    return Buffer.concat(parts);
}
...
function printTcpOnce(ip: string, port: number, data: Buffer): Promise<void> {
    return new Promise((resolve, reject) => {
        ...
        let dataSent = false; // write() callback muvaffaqiyatli chaqirildi
        let settled = false;
        const done = (err?: Error) => {
            ...
            if (err) {
                reject(new Error(`Printer xatosi (${ip}:${port}) - ${err.message}. Printer o'chiq yoki IP xato kiritilgan.`));
            } else {
                resolve();
            }
        };
        ...
        socket.on("error", (err) => {
            if (dataSent) {
                // Data kernel buferiga yetib bordi; printer TCP ulanishni yopganda RST kelishi
                // mumkin — bu haqiqiy xato emas, chop etish muvaffaqiyatli yakunlandi.
                if (!settled) { settled = true; try { socket.destroy(); } catch {} resolve(); }
            } else {
                done(err);
            }
        });
        socket.connect(port, ip, () => {
            ...
            socket.write(data, (err) => {
                ...
                dataSent = true;
                // Printer barcha ma'lumotni kernel buferidan o'qib, chop etib, kesishi uchun 2.5s kutamiz
        ...
        return printQueue.run(async () => {
            try {
                ...
                if (job.printerIp.startsWith("usb://")) {
                    ...
                    console.log(`[PrinterService] Sending TCP print to ${job.printerIp}:${port}`);
                    await printTcp(job.printerIp, port, buffer); // retry bilan
                    console.log(`[PrinterService] TCP print SUCCESS to ${job.printerIp}:${port}`);
                }
                return { success: true };
            } catch (err: unknown) {
