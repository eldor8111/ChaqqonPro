...
export default function OmborKirimPage() {
    ...
    const loadProducts = useCallback(async () => {
        try {
            ...
            const [xomRes, menuRes] = await Promise.all([
                fetch("/api/smart/xomashyo"),
                fetch("/api/smart/menu?all=1"),
            ]);
    ...
    const fetchSuppliers = useCallback(async () => {
        try {
            const res = await fetch("/api/smart/kontragent");
            if (res.ok) {
    ...
    const fetchUsdRate = useCallback(async () => {
        try {
            const res = await fetch("/api/smart/moliya");
            if (res.ok) {
                const data = await res.json();
    ...
    const handleBulkDelete = async () => {
        ...
        try {
            ...
            for (const docId of Array.from(selectedIds)) {
                await fetch("/api/smart/ombor/kirim", {
                    method: "DELETE",
    ...
    const fetchKirimlar = async () => {
        setIsLoading(true);
        try {
            const res = await fetch("/api/smart/ombor/kirim");
