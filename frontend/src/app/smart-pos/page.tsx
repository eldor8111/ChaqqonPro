...
function PayModal({ total, onPay, onClose, loading, servicePct = 0 }: { total: number; onPay: (m: string, customerId?: string) => void; onClose: () => void; loading: boolean; servicePct?: number }) {
    ...
    useEffect(() => {
        if (method === "qarz") {
            const { kassirSession, deviceSession } = useStore.getState();
            const tok = kassirSession?.token || deviceSession?.token;
            fetch("/api/customers", { headers: tok ? { Authorization: `Bearer ${tok}` } : {} })
    ...
    const handleCreateCustomer = async () => {
        ...
        try {
            const { kassirSession, deviceSession } = useStore.getState();
            const tok = kassirSession?.token || deviceSession?.token;
            const custHeaders: Record<string,string> = { "Content-Type": "application/json" };
            if (tok) custHeaders["Authorization"] = `Bearer ${tok}`;
...
function MenuPanel({ onConfirm, onPay, kassirPrinterIp, autoPrintReceipt, instantAdd, servicePct = 0, tableName = "Buyurtma" }: {
    ...
    const handlePay = async (method: string, customerId?: string) => {
        ...
        if (!finalIp) {
            try {
                const pToken = store.kassirSession?.token || store.deviceSession?.token;
                const res = await fetch("/api/smart/printers", { headers: pToken ? { Authorization: `Bearer ${pToken}` } : {} });
                if (res.ok) {
        ...
        if (autoPrintReceipt !== false) {
            if (!finalIp) {
                setTimeout(() => alert("⚠️ Printer topilmadi!\nAdmin panelda kassir profilida 'Printer IP' ni kiriting.\nMasalan: 192.168.1.100"), 300);
            } else {
                const now = new Date();
                const timeStr = `${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}`;
                const printToken = store.kassirSession?.token || store.deviceSession?.token;
                const printHeaders: Record<string, string> = { "Content-Type": "application/json" };
                if (printToken) printHeaders["Authorization"] = `Bearer ${printToken}`;
                fetch("/api/smart/print", {
                    method: "POST",
                    ...
                    if (!r.ok) {
                        ...
                        setTimeout(() => alert(`⚠️ Chek chiqmadi!\n${d.error || 'Printer bilan ulanishda xato'}\nPrinter IP: ${finalIp}:9100`), 300);
                    }
                }).catch(e => setTimeout(() => alert(`⚠️ Chek chiqmadi!\n${e.message || 'Tarmoq xatosi'}\nPrinter IP: ${finalIp}:9100`), 300));
            }
        }
...
export default function UbtPosPage() {
    ...
    const fetchPrinterStatus = () => {
        const { kassirSession, deviceSession } = useStore.getState();
        const token = kassirSession?.token || deviceSession?.token;
        const headers: Record<string, string> = token ? { Authorization: `Bearer ${token}` } : {};
        fetch("/api/smart/printers/ping", { headers }).then(r => r.json()).then(data => {
    ...
    useEffect(() => {
        ...
        // Fast polling for orders (15 seconds)
        const ti = setInterval(() => { store.fetchSmartTables(); fetchTwAndDlOrders(); }, 15000);
