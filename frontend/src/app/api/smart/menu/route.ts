...
export async function GET(request: NextRequest) {
    try {
        ...
        // Local POS fallback: agar auth bo'lmasa, bazadagi yagona active tenantni ishlatamiz.
        // Bu local desktop POS uchun xavfsiz — internet bilan bog'liq emas.
        if (!tenantId) {
            const fallback = await prisma.tenant.findFirst({
                where: { status: "active" },
                select: { id: true }
            });
            if (!fallback) return NextResponse.json({ categories: [], items: [], cancelCode: "" });
