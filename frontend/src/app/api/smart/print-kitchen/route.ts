    ...
    try {
        ...
        if (ids.length > 0) {
            products = await prisma.$queryRawUnsafe(
                `SELECT id, printerIp FROM Product WHERE tenantId = ? AND id IN (${ids.map(() => "?").join(",")})`,
                auth.tenantId, ...ids
            );
        }
        ...
        try {
            const fp: any[] = await prisma.$queryRawUnsafe(
                `SELECT ipAddress, role FROM SmartPrinter WHERE tenantId = ? AND (isActive IS NULL OR isActive = 1) ORDER BY (role = 'kitchen') DESC, createdAt ASC LIMIT 1`,
                auth.tenantId
            );
