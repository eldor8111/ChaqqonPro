...
export async function POST(request: NextRequest) {
    try {
        ...
        // Extract printerIp and autoPrintReceipt from phone JSON
        let printerIp = "";
        let autoPrintReceipt = true;
        try {
            const p = staff.phone ? JSON.parse(staff.phone) : {};
            printerIp = p.printerIp || staff.printerIp || "";
            if (p.autoPrintReceipt !== undefined) autoPrintReceipt = p.autoPrintReceipt;
        } catch {
            printerIp = staff.printerIp || "";
        }

        // Fallback: SmartPrinter jadvalidan birinchi printerni olish
        if (!printerIp) {
            try {
                ...
                if (printers.length > 0) printerIp = printers[0].ipAddress || "";
            } catch {}
        }
